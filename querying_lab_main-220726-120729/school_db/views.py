...
def problem_one(request):
  greater_gpa = Student.objects.filter(gpa__gt = 3).order_by('-gpa')
  
  for student in greater_gpa:
...
def problem_two(request):
  instructor_hire_date = Instructor.objects.filter(hire_date__lt='2010-01-01').order_by('hire_date')

  for instructor in instructor_hire_date:
...
def problem_three(request):
  ...
  instructor_course = Course.objects.filter(instructor_id=2)
...
def problem_five(request):
  new_student = Student.objects.create(first_name = "Mike", last_name = "Jones", year = 9, gpa = 1.5)
  student_id = 11
...
def problem_six(request):
  update_student = Student.objects.filter(id = '11').update(gpa = '2.1')
  student_id = '11'
  for student_id in update_student:
    print(f"ID: {student_id},\nFull Name: {update_student.first_name} {update_student.last_name},\nYear: {update_student.year},\nGPA: {update_student.gpa}")
